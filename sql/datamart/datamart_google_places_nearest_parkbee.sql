CREATE OR REPLACE TABLE
  `grand-water-473707-r8.datamart.datamart_google_places_nearest_parkbee` AS
WITH
  dim_fact_google AS (
  SELECT
    dgp.name,
  ...
  FROM
    `grand-water-473707-r8.dwh.fact_parking_candidate_locations` fpcl
  INNER JOIN
    `grand-water-473707-r8.dwh.dim_google_places` dgp
  ON
    dgp.place_id = fpcl.place_id ),
  join_parkbee AS (
  SELECT
    dfg.place_id,
    ...
    -- Row numbering to take NEAREST ParkBee only
    ROW_NUMBER() OVER (PARTITION BY dfg.place_id ORDER BY ST_DISTANCE(dfg.geom, dpl.geom) ) AS rn
  FROM
    dim_fact_google dfg
  CROSS JOIN
